...
public class TestCombinedApplication {
    ...
    private static void initializeDefaultAirports() {
        airports.add(new Airport("ElVen", "Greece","Athens",33.2313, -123.3245));
    ...
    public static void showNumberOfDestinationsWindow() {
        ...
        submitButton.addActionListener(e -> {
            try {
                ...
                if (numberOfDestinations > 0 && numberOfDestinations <= insertedAirports.size()) {
                    ...
                    JOptionPane.showMessageDialog(frame, "Please enter a number between 1 and " + insertedAirports.size());
                }
            } catch (NumberFormatException ex) {
                JOptionPane.showMessageDialog(frame, "Please enter a valid number!");
            }
    ...
    public static void calculateDistancesAndAssignments(int[] visits) {
        ...
        double[] mockLatitudes = new double[airports.size()];
        double[] mockLongitudes = new double[airports.size()];

        // Populate mockLatitudes and mockLongitudes
        for (int i = 0; i < airports.size(); i++) {
            mockLatitudes[i] = airports.get(i).getLatitude(); // Get latitude
            mockLongitudes[i] = airports.get(i).getLongitude(); // Get longitude
        }
